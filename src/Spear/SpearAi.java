...
public class SpearAi {
	...
	TurnInformation ti=new TurnInformation(tir.getTurnInformation(),tir.getBattleField());//获取战场信息
...
String killCourt(Samurai samurai, String killString) {	//走一步杀死 TODO Auto-generated method stub
	...
	public void analyseMe() {//必躲：分析自己的位置
		...
		a[1]=within(enspear,row,col);
		a[2]=within(ensword,row,col);
		a[3]=within(enbattleax,row,col);
		if (a[1]&&a[2]&&a[0]) {
